...
namespace LoanManagement.Services.Tests.Unit
{
    public class InMemoryDataBase : IDisposable
    {
        ...
        private ConstructorInfo? FindSuitableConstructor<TDbContext>() where
        ...
        private Func<TDbContext> ResolveFactory<TDbContext>() where TDbContext
            : DbContext
        {
            ...
            if (constructor == null)
            {
                throw new Exception($"no constructor found on '{typeof(
                    TDbContext).Name}' " +
                                    "with one parameter of type " +
                                    $"DbContextOptions<{typeof(TDbContext).
